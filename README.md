...
npm install
npm run dev
# → http://localhost:3000 (Next 앱은 kbeauty-dashboard-v2 워크스페이스에서 실행)
```
...
3. **Root Directory**를 **`kbeauty-dashboard-v2`** 로 지정하세요. (Next 앱이 이 폴더에 있습니다.) 예전에 레포 루트(`.`)나 `deploy`로 지정했다면 반드시 바꾼 뒤 저장하고 다시 배포하세요.
...
> 이 URL을 아는 사람은 누구나 **열람 및 수정** 가능합니다 (Link-Only 접근). 진짜 인증이 필요하면 `kbeauty-dashboard-v2/src/app/page.jsx`에 간단한 비밀번호 게이트를 추가할 수 있습니다.
...
### A. 간단한 비밀번호 보호
`kbeauty-dashboard-v2/src/app/page.jsx`에서 prompt로 비밀번호 체크 추가 (5분 작업)
...
현재 광고 데이터 (Naver/Google/Meta)와 초기 블로그 90일치는 `kbeauty-dashboard-v2/src/data/seed.js`에 하드코딩되어 있습니다. 매주 업데이트하려면:
...
```
kbeauty-dashboard-v2/        - Next.js 앱 (Vercel Root Directory)
├── src/
│   ├── app/
│   │   ├── layout.jsx       - 루트 레이아웃
│   │   ├── page.jsx         - 진입점
...
- Vercel에서 `No Output Directory named "public"` 이면, 프로젝트가 **Next가 아니라 정적 사이트**로 잡힌 경우입니다. **Settings → Build & Deployment**에서 **Framework Preset**을 **Next.js**로 두고, **Output Directory**는 **비우기**(Override 해제)하세요. **Root Directory**는 **`kbeauty-dashboard-v2`** 여야 하며, 해당 폴더의 `vercel.json`이 Next 빌드를 가리킵니다.
```bash
cd kbeauty-dashboard-v2
